'''wxPython GUI for finding and renaming similar images'''
...
# Custom event for image renaming
ImageRenameEvent, IMAGE_EVT_RENAME = wx.lib.newevent.NewEvent()

class ImageDisplayPanel(wx.Panel):
    ''' Entry / item for displaying the image and details '''
    def __init__(self, parent, image_object, image, property_diffs=None, group_paths=None):
        ...
        # Rename button (far right) - only show if there are other files to rename from
        if len(self.group_paths) > 1:
            rename_btn = wx.Button(self, label="Rename")
            rename_btn.Bind(wx.EVT_BUTTON, self.on_rename)
            sizer.Add(rename_btn, 0, wx.ALIGN_CENTER_VERTICAL|wx.RIGHT, 5)
        
        self.SetSizer(sizer)
    
    def on_rename(self, event):
        """Rename this file using the stem of another file in the same group"""
        current_path = Path(self.image_object.file_path)
        # print(str(current_path))
        
        # Find another file in the group (not the current one)
        other_files = [Path(p) for p in self.group_paths if str(p) != str(current_path)]
        
        if not other_files:
            wx.MessageBox("No other files in group to use for renaming", "Error", wx.OK | wx.ICON_ERROR)
            return
        
        if len(other_files) > 1:
            wx.MessageBox("More than 1 other file in group to use for renaming", "Error", wx.OK | wx.ICON_ERROR)
        
        # print(str(other_files[0]))
        # print(str(other_files[1]))
        
        # Use the first other file's stem
        source_stem = other_files[0].stem
        new_name = f"{source_stem}{current_path.suffix}"
        new_path = current_path.parent / new_name
        
        if new_path.exists():
            wx.MessageBox(f"Target file already exists: {new_path}", "Error", wx.OK | wx.ICON_ERROR)
            return
        ...
        try:
            ...
            # Update the path text to show the new path
            for child in self.GetChildren():
                if isinstance(child, wx.Panel):
                    for grandchild in child.GetChildren():
                        if isinstance(grandchild, wx.StaticText):
                            grandchild.SetLabel(str(new_path))
                            grandchild.Wrap(180)
                            break
            
            # Send rename event up to parent window
            rename_event = ImageRenameEvent()
            wx.PostEvent(self.GetParent().GetParent(), rename_event)
...
class GroupWindow(wx.Frame):
    ...
    def __init__(self, group_num, image_paths, image_objects, image_thumbs, total_groups=None, fast_mode=False):
        ...
        for image_path in image_paths:
            if image_path in image_thumbs and image_thumbs[image_path]:
                image_panel = ImageDisplayPanel(
                    ...
                    self.property_diffs,
                    image_paths  # Pass all group paths for renaming reference
                )
        ...
        # Track fast mode and renaming counts
        self.fast_mode = fast_mode
        self.total_images = len(image_paths)
        self.renamed_count = 0
        
        # Bind rename event handler
        self.Bind(IMAGE_EVT_RENAME, self.on_image_renamed)
    ...
    def on_image_renamed(self, event):
        """Handle image renaming events"""
        self.renamed_count += 1
        if self.fast_mode and abs(self.total_images - self.renamed_count) <= 1:
            self.Close()

def main():
    parser = argparse.ArgumentParser(description='Scan for similar image files and rename them')
    ...
    parser.add_argument('--fast-mode', action='store_true',
                       help='Auto-close group window when all but one images are renamed')
    ...
    # Filter out groups with more than 2 members
    filtered_groups = {k: v for k, v in grouped_images.items() if len(v) <= 2}
    print(f"Groups with 2 or fewer members: {len(filtered_groups)}")

    # Create wxPython app
    app = wx.App(False)
    # Show each group in wxPython window
    total_groups = len(filtered_groups.keys())
    for group_num in sorted(filtered_groups.keys()):
        if filtered_groups[group_num]:
            GroupWindow(
                group_num,
                filtered_groups[group_num],
                image_objects,
                image_thumbs,
                total_groups,
                args.fast_mode
